use super::{r1cs_to_qap::R1CSToQAP, LegoGroth16, ProvingKey, Vec, VerifyingKey};
use crate::mpc::spdz_pairing::MpcPairingTrait;
use crate::snark::legogroth16::link::SubspaceSnark;
use crate::snark::legogroth16::link::{PESubspaceSnark, SparseMatrix, PP};
    ...
    E: MpcPairingTrait<B>,
{
    ...
    pub fn generate_random_parameters_with_reduction<C>(
        circuit: C,
        pedersen_bases: &[<E as MpcPairingTrait<B>>::G1Affine],
        rng: &mut impl Rng,
    ) -> R1CSResult<ProvingKey<B, E>>
    where
        C: ConstraintSynthesizer<<E as MpcPairingTrait<B>>::ScalarField>,
    {
        let alpha = <E as MpcPairingTrait<B>>::ScalarField::rand(rng);
        let beta = <E as MpcPairingTrait<B>>::ScalarField::rand(rng);
        let gamma = <E as MpcPairingTrait<B>>::ScalarField::rand(rng);
        let delta = <E as MpcPairingTrait<B>>::ScalarField::rand(rng);

        let g1_generator = <E as MpcPairingTrait<B>>::G1::rand(rng);
        let g2_generator = <E as MpcPairingTrait<B>>::G2::rand(rng);

        // LegoGroth16 additions
        let eta = <E as MpcPairingTrait<B>>::ScalarField::rand(rng);
    ...
    pub fn generate_parameters_with_qap<C>(
        circuit: C,
        alpha: <E as MpcPairingTrait<B>>::ScalarField,
        beta: <E as MpcPairingTrait<B>>::ScalarField,
        gamma: <E as MpcPairingTrait<B>>::ScalarField,
        delta: <E as MpcPairingTrait<B>>::ScalarField,
        eta: <E as MpcPairingTrait<B>>::ScalarField,
        g1_generator: <E as MpcPairingTrait<B>>::G1,
        g2_generator: <E as MpcPairingTrait<B>>::G2,
        pedersen_bases: &[<E as MpcPairingTrait<B>>::G1Affine],
        rng: &mut impl Rng,
    ) -> R1CSResult<ProvingKey<B, E>>
    where
        C: ConstraintSynthesizer<<E as MpcPairingTrait<B>>::ScalarField>,
    {
        ...
        let scalar_bits = <E as MpcPairingTrait<B>>::ScalarField::MODULUS_BIT_SIZE as usize;
        ...
        let g2_table = FixedBase::get_window_table::<<E as MpcPairingTrait<B>>::G2>(
            scalar_bits,
            g2_window,
            g2_generator,
        );
        ...
        let b_g2_query =
            FixedBase::msm::<<E as MpcPairingTrait<B>>::G2>(scalar_bits, g2_window, &g2_table, &b);
        drop(g2_table);
        end_timer!(b_g2_time);
        ...
        let g1_table = FixedBase::get_window_table::<<E as MpcPairingTrait<B>>::G1>(
            scalar_bits,
            g1_window,
            g1_generator,
        );
        ...
        let a_query =
            FixedBase::msm::<<E as MpcPairingTrait<B>>::G1>(scalar_bits, g1_window, &g1_table, &a);
        drop(a);
        end_timer!(a_time);
        ...
        let b_g1_query =
            FixedBase::msm::<<E as MpcPairingTrait<B>>::G1>(scalar_bits, g1_window, &g1_table, &b);
        drop(b);
        end_timer!(b_g1_time);

        // Compute the H-query
        let h_time = start_timer!(|| "Calculate H");
        let h_query = FixedBase::msm::<<E as MpcPairingTrait<B>>::G1>(
            scalar_bits,
            g1_window,
            &g1_table,
            &QAP::h_query_scalars::<_, D<<E as MpcPairingTrait<B>>::ScalarField>>(
                m_raw - 1,
                t,
                zt,
                delta_inverse,
        ...
        let l_query =
            FixedBase::msm::<<E as MpcPairingTrait<B>>::G1>(scalar_bits, g1_window, &g1_table, &l);
        drop(l);
        end_timer!(l_time);
        ...
        let gamma_abc_g1 = FixedBase::msm::<<E as MpcPairingTrait<B>>::G1>(
            scalar_bits,
            g1_window,
            &g1_table,
            &gamma_abc,
        ...
        let a_query = <E as MpcPairingTrait<B>>::G1::normalize_batch(&a_query);
        let b_g1_query = <E as MpcPairingTrait<B>>::G1::normalize_batch(&b_g1_query);
        let b_g2_query = <E as MpcPairingTrait<B>>::G2::normalize_batch(&b_g2_query);
        let h_query = <E as MpcPairingTrait<B>>::G1::normalize_batch(&h_query);
        let l_query = <E as MpcPairingTrait<B>>::G1::normalize_batch(&l_query);
        end_timer!(batch_normalization_time);
        end_timer!(setup_time);

        // LegoGroth16 additions

        let eta_delta_inv_g1 = g1_generator.mul(eta * &delta_inverse).into_affine();
        ...
        // link_pp is the configuration for the link subspace snark
        let link_pp =
            PP::<<E as MpcPairingTrait<B>>::G1Affine, <E as MpcPairingTrait<B>>::G2Affine> {
                l: link_rows,
                t: link_cols,
                g1: <E as MpcPairingTrait<B>>::G1Affine::generator(),
                g2: <E as MpcPairingTrait<B>>::G2Affine::generator(),
            };

        // Build a matrix M as in equation (1) of the paper
        let mut link_m =
            SparseMatrix::<<E as MpcPairingTrait<B>>::G1Affine>::new(link_rows, link_cols);
        ...
        let (link_ek, link_vk) = PESubspaceSnark::<B, E>::keygen(rng, &link_pp, link_m);

        let vk = VerifyingKey::<B, E> {
            alpha_g1: alpha_g1.into_affine(),
            beta_g2: beta_g2.into_affine(),
            gamma_g2: gamma_g2.into_affine(),
            delta_g2: delta_g2.into_affine(),
            gamma_abc_g1: <E as MpcPairingTrait<B>>::G1::normalize_batch(&gamma_abc_g1),

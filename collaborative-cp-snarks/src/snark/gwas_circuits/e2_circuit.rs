...
/// Circuit to compute E_j^{aa} = (1 - alpha_j)^2 * (n - sum_y)
#[derive(Clone)]
pub struct E2Circuit<F: Field> {
...
impl<F: Field> ConstraintSynthesizer<F> for E2Circuit<F> {
    fn generate_constraints(
        ...
        // Compute (1 - alpha)^2
        let one_minus_alpha_squared_value = one_minus_alpha_value.square();
        let one_minus_alpha_squared = cs.new_witness_variable(|| Ok(one_minus_alpha_squared_value))?;
        cs.enforce_constraint(
            lc!() + one_minus_alpha,
            lc!() + one_minus_alpha,
            lc!() + one_minus_alpha_squared,
        )?;

        // Compute E_aa = (1 - alpha)^2 * (n - sum_y)
        let E_aa_value = one_minus_alpha_squared_value * n_minus_sum_y_value;
...
mod tests {
    ...
    fn test_e2_circuit() {
        ...
        // Compute E_j^{aa} = (1 - alpha)^2 * (n - sum_y)
        let one = Fr::one();
        ...
        let E_aa_value = one_minus_alpha_squared * n_minus_sum_y;

        // Create the circuit
        let circuit = E2Circuit {
            y_list,
            n,

...
/// Circuit to compute E_j^{Aa} = 2 * alpha_j * (1 - alpha_j) * (n - sum_y)
#[derive(Clone)]
pub struct E1Circuit<F: Field> {
...
impl<F: Field> ConstraintSynthesizer<F> for E1Circuit<F> {
    fn generate_constraints(
        ...
        // Compute 1 - alpha
        let one_minus_alpha_value = F::one() - alpha_value;
        let one_minus_alpha = cs.new_witness_variable(|| Ok(one_minus_alpha_value))?;
        cs.enforce_constraint(
            lc!() + (F::one(), Variable::One) - alpha,
            lc!() + Variable::One,
            lc!() + one_minus_alpha,
        )?;

        // Compute alpha * (1 - alpha)
        let alpha_times_one_minus_alpha_value = alpha_value * one_minus_alpha_value;
        let alpha_times_one_minus_alpha = cs.new_witness_variable(|| Ok(alpha_times_one_minus_alpha_value))?;
        cs.enforce_constraint(
            lc!() + alpha,
            lc!() + one_minus_alpha,
            lc!() + alpha_times_one_minus_alpha,
        )?;

        // Compute 2 * alpha * (1 - alpha)
        let two = F::from(2u64);
        let two_alpha_times_one_minus_alpha_value = two * alpha_times_one_minus_alpha_value;
        let two_alpha_times_one_minus_alpha = cs.new_witness_variable(|| Ok(two_alpha_times_one_minus_alpha_value))?;
        cs.enforce_constraint(
            lc!() + (two, Variable::One),
            lc!() + alpha_times_one_minus_alpha,
            lc!() + two_alpha_times_one_minus_alpha,
        )?;

        // Compute E_aa = 2 * alpha * (1 - alpha) * (n - sum_y)
        let E_aa_value = two_alpha_times_one_minus_alpha_value * n_minus_sum_y_value;
...
mod tests {
    ...
    fn test_e1_circuit() {
        ...
        // Compute E_j^{Aa} = 2 * alpha * (1 - alpha) * (n - sum_y)
        let one = Fr::one();
        let one_minus_alpha = one - alpha_value;
        let alpha_times_one_minus_alpha = alpha_value * one_minus_alpha;
        let two = Fr::from(2u64);
        let two_alpha_times_one_minus_alpha = two * alpha_times_one_minus_alpha;
        let n_f = Fr::from(n as u64);
        let n_minus_sum_y = n_f - sum_y_value;
        let E_aa_value = two_alpha_times_one_minus_alpha * n_minus_sum_y;

        // Create the circuit
        let circuit = E1Circuit {
            y_list,
            n,

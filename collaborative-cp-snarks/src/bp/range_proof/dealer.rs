...
use crate::bp::inner_product_proof;
use crate::bp::range_proof::RangeProof;
use crate::bp::transcript_bp::{TranscriptProtocol, BPTranscript};
...
impl<G:Group> Dealer<G> {
    ...
    pub fn new<'a, 'b>(
        ...
        transcript: &'a mut BPTranscript<G>,
        n: usize,
        m: usize,
...
pub struct DealerAwaitingBitCommitments<'a, 'b, G:Group> {
    bp_gens: &'b BulletproofGens<G>,
    pc_gens: &'b PedersenGens<G>,
    transcript: &'a mut BPTranscript<G>,
    /// The dealer keeps a copy of the initial transcript state, so
    /// that it can attempt to verify the aggregated proof at the end.
    initial_transcript: BPTranscript<G>,
    n: usize,
    m: usize,
}

impl<'a, 'b, G:Group> DealerAwaitingBitCommitments<'a, 'b, G> {
    ...
    pub fn receive_bit_commitments(
        ...
        let A: G = bit_commitments.iter().map(|vc| vc.A_j).sum();
        self.transcript.append_point(b"A", &A);

        let S: G = bit_commitments.iter().map(|vc| vc.S_j).sum();
...
pub struct DealerAwaitingPolyCommitments<'a, 'b, G:Group> {
    n: usize,
    m: usize,
    transcript: &'a mut BPTranscript<G>,
    initial_transcript: BPTranscript<G>,
...
impl<'a, 'b, G:Group> DealerAwaitingPolyCommitments<'a, 'b, G> {
    ...
    pub fn receive_poly_commitments(
        ...
        let T_1: G = poly_commitments.iter().map(|pc| pc.T_1_j).sum();
        let T_2: G = poly_commitments.iter().map(|pc| pc.T_2_j).sum();
...
/// [`RangeProof`].
pub struct DealerAwaitingProofShares<'a, 'b, G:Group> {
    n: usize,
    m: usize,
    transcript: &'a mut BPTranscript<G>,
    initial_transcript: BPTranscript<G>,
...
impl<'a, 'b, G:Group> DealerAwaitingProofShares<'a, 'b, G> {
    ...
    fn assemble_shares(&mut self, proof_shares: &[ProofShare<G>]) -> Result<RangeProof<G>, MPCError> {
        ...
        let G_factors: Vec<G::ScalarField> = iter::repeat(G::ScalarField::one()).take(self.n * self.m).collect();
        let H_factors: Vec<G::ScalarField> = util::exp_iter(self.bit_challenge.y.inverse().unwrap())
            .take(self.n * self.m)
            .collect();
    ...
    pub fn receive_shares_with_rng<R: RngCore>(
        mut self,
            ...
            .is_ok()
        {
            ...
            // FIXME: audit not implemented yet

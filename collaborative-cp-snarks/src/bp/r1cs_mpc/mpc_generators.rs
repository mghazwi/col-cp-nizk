...
extern crate alloc;
use alloc::vec::Vec;
use ark_ec::{AffineRepr, Group, CurveConfig, CurveGroup, VariableBaseMSM};
...
impl PedersenGens {
    ...
    pub fn commit(&self, value: SF<Fr>, blinding: SF<Fr>) -> SG<G> {
        let bv = (self.B * value).into_affine();
        let bb_vb = (self.B_blinding * blinding).into_affine();
        // let c = <SG<G> as AffineRepr>::Group::msm(&[self.B,self.B_blinding], &[value,blinding]).unwrap().into_affine();
        let c = (bv + bb_vb).into_affine();
        c
    }
}
...
impl Iterator for GeneratorsChain {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // let s = SF::<Fr>::rand(&mut rng);
        Some((SG::<G>::rand(&mut rng)))//generator()*s).into_affine())
    }
...
pub struct BulletproofGens {
    ...
    pub G_vec: Vec<Vec<SG<G>>>,
    /// Precomputed \\(\mathbf H\\) generators for each party.
    pub H_vec: Vec<Vec<SG<G>>>,
}
...
impl BulletproofGens {
    ...
    pub fn increase_capacity(&mut self, new_capacity: usize) {
        ...
        for i in 0..self.party_capacity {
            ...
            self.G_vec[i].extend(
                &mut GeneratorsChain::new(party_index)
    ...
    pub(crate) fn G(&self, n: usize, m: usize) -> impl Iterator<Item = &SG<G>> {
        AggregatedGensIter {
            n,
            m,
            array: &self.G_vec,
    ...
    pub(crate) fn H(&self, n: usize, m: usize) -> impl Iterator<Item = &SG<G>> {
        AggregatedGensIter {
            n,
            m,
            array: &self.H_vec,

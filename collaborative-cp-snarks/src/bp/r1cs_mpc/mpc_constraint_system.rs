...
use super::mpc_linear_combination::{LinearCombination, Variable};
...
pub trait ConstraintSystem {
    ...
    fn allocate(&mut self, assignment: Option<SF<Fr>>) -> Result<Variable, R1CSError>;
...
pub trait RandomizedConstraintSystem: ConstraintSystem {
    ...
    /// })
    /// ```
    fn challenge_scalar(&mut self, label: &'static [u8]) -> SF<Fr>;
}